...
Point2f Source[] = {Point2f(55,150), Point2f(342,150), Point2f(25,200), Point2f(385,200)};
...
void Threshold()
{
	...
	inRange(frameGray, 215, 255, frameThresh);
	Canny(frameGray, frameEdge, 150, 480, 3, false);
	add(frameThresh, frameEdge, frameFinal);
	cvtColor(frameFinal, frameFinal, COLOR_GRAY2RGB);
	cvtColor(frameFinal, frameFinalDuplicate, COLOR_RGB2BGR); //used in histo function only
	cvtColor(frameFinal, frameFinalDuplicate1, COLOR_RGB2BGR); //used in histo function only
...
void LaneFinder()
{
    ...
    LeftPtr = max_element(histrogramLane.begin(), histrogramLane.begin() + 165);
    LeftLanePos = distance(histrogramLane.begin(), LeftPtr); 
    
    vector<int>:: iterator RightPtr;
    RightPtr = max_element(histrogramLane.begin() +265, histrogramLane.end());
...
void Stop_detection()
{
	...
	dist_Stop = (-0.385)*(P2.x-P1.x) + 46.923;
	
	ss.str(" ");
	ss.clear();
	ss<<"Distance = "<<dist_Stop<<" cm";
	putText(RoI_Stop, ss.str(), Point2f(1,50), 0,1, Scalar(0,0,255), 2);
...
void Object_detection()
{
	...
	putText(RoI_Object, "Car Detected", P1, FONT_HERSHEY_PLAIN, 1,  Scalar(0, 0, 255, 255), 2);
	dist_Object = (-0.42)*(P2.x-P1.x) + 69.6;
	
	ss.str(" ");
	ss.clear();
	ss<<"D = "<<dist_Object<<" cm";
	putText(RoI_Object, ss.str(), Point2f(1,50), 0,1, Scalar(0,0,255), 2);
